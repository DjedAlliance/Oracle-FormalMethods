...
Fixpoint modify_list (type : Type) 
    ...
    | (addr, oldElem) :: targetList' => 
        ...
        then current_list ++ ((addr, elem) :: nil) ++ targetList'
        else modify_list type targetList' targetAddress elem (current_list ++ ((addr, oldElem) :: nil))
    end.
...
Fixpoint get_consumer_info (allConsumers : list (address * ConsumerInfo)) 
    ...
    | nil => Build_ConsumerInfo 0 0 0 0 0
    | (addr, info) :: allConsumers' =>
        if compare_address addr targetAddress
        then info 
        else get_consumer_info allConsumers' targetAddress
    end.
